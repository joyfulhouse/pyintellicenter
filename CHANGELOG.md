...
- **Schedule control methods** on `ICModelController`:
  - `set_schedule_enabled(sched_objnam, enabled)` - Enable or disable a schedule
  - `is_schedule_enabled(sched_objnam)` - Check if a schedule is enabled
  - `is_schedule_active(sched_objnam)` - Check if a schedule is currently running
