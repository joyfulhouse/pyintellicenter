...
class TestICModelController:
    ...
    async def test_set_schedule_enabled(self, controller, model):
        """Test set_schedule_enabled convenience method."""
        controller._connection = MagicMock()
        controller._connection.connected = True
        controller._connection.send_request = AsyncMock(
            return_value={"response": "200", "objectList": []}
        ...
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "OFF", "ACT": "OFF"},
        )

        await controller.set_schedule_enabled("SCH01", True)

        # Verify the command was sent with correct parameters
        controller._connection.send_request.assert_called()
        call_args = controller._connection.send_request.call_args
        assert call_args[0][0] == "SETPARAMLIST"
        assert call_args[1]["objectList"][0]["objnam"] == "SCH01"
        assert call_args[1]["objectList"][0]["params"]["STATUS"] == "ON"

    async def test_set_schedule_enabled_off(self, controller, model):
        """Test set_schedule_enabled with enabled=False."""
        controller._connection = MagicMock()
        controller._connection.connected = True
        controller._connection.send_request = AsyncMock(
            return_value={"response": "200", "objectList": []}
        ...
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "ON", "ACT": "ON"},
        )
    ...
    def test_is_schedule_enabled(self, controller, model):
        """Test is_schedule_enabled getter method."""
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "ON", "ACT": "OFF"},
        )

        assert controller.is_schedule_enabled("SCH01") is True

    def test_is_schedule_enabled_false(self, controller, model):
        """Test is_schedule_enabled returns False when disabled."""
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "OFF", "ACT": "OFF"},
        )
    ...
    def test_is_schedule_enabled_not_found(self, controller):
        """Test is_schedule_enabled returns False when schedule not found."""
        assert controller.is_schedule_enabled("NONEXISTENT") is False

    def test_is_schedule_active(self, controller, model):
        """Test is_schedule_active getter method."""
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "ON", "ACT": "ON"},
        )

        assert controller.is_schedule_active("SCH01") is True

    def test_is_schedule_active_false(self, controller, model):
        """Test is_schedule_active returns False when not running."""
        model.add_object(
            "SCH01",
            {"OBJTYP": "SCHED", "SNAME": "Pool Filter", "STATUS": "ON", "ACT": "OFF"},
        )

        assert controller.is_schedule_active("SCH01") is False

    def test_is_schedule_active_not_found(self, controller):
        """Test is_schedule_active returns False when schedule not found."""
        assert controller.is_schedule_active("NONEXISTENT") is False
